...
<section class="packages" id="packages">
    ...
    <h1 class="heading"><span>المسارات المؤرشفة</span></h1>

    <div class="box-container">

        @foreach($archived_paths as $archived_path)
      <div class="box" data-aos="fade-up">
        <div class="image">
            <img src="{{ $archived_path->image_url }}" alt="">
            <h3><i class="fas fa-map-marker-alt"></i> {{ $archived_path->area->name }} </h3>
        </div>
        <div class="content">
            <h1><i class="fas fa-calendar-day"></i> {{ date('Y-m-d', strtotime($archived_path->date)) }}</h1>
            <h2><i class="fas fa-clock"></i> {{ date('h:i A', strtotime($archived_path->start)) }} إلى {{ date('h:i A', strtotime($archived_path->end)) }}</h2>
            <h2><i class="fas fa-road"></i> {{ $archived_path->distance }} كم</h2>
            <p>{{ implode(' ', array_slice(preg_split('/\s+/', $archived_path->description), 0, 15)) }}</p>
