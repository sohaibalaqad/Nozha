        ...
        @for($i = 1; $i <= 5; $i++)
            <div class="input-group p-2">
                <div class="custom-file">
                    ...
                    <label class="custom-file-label" for="inputGroupFile{{$i}}">أضف صورة {{$i}} للمسار </label>
                </div>
            </div>
        @endfor
...
{{--        </div>--}}
        ...
        <div class="form-group">
            {{ Form::label('الحالة') }}
            {{ Form::select('status', ['1' => 'نشط', '0' => 'غير نشط'], $path->status,   ['class' => 'form-control' . ($errors->has('status') ? ' is-invalid' : ''), 'placeholder' => 'الحالة']) }}
